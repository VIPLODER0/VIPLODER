...
# Bot Configuration
TELEGRAM_BOT_TOKEN = '8012442954:AAHi6WP2ypYhgHGdya_j6JvQnHj5shuLYEM'
...
async def attack(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Fetch max attack limit from database
    max_attacks_setting = settings_collection.find_one({"setting": "max_attacks_per_ip_port"})
    MAX_ATTACKS_PER_IP_PORT = max_attacks_setting["value"] if max_attacks_setting else 3  # Default is 3
    ...
    # Check if the user has already attacked this IP and port too many times
    if user_id in user_attack_history:
        if (ip, port) in user_attack_history[user_id]:
            if user_attack_history[user_id][(ip, port)] >= MAX_ATTACKS_PER_IP_PORT:
                await context.bot.send_message(chat_id=chat_id, text="*❌ Attack limit reached for this IP and port*", parse_mode='Markdown')
                return
            else:
                user_attack_history[user_id][(ip, port)] += 1
        else:
            user_attack_history[user_id][(ip, port)] = 1
    else:
        user_attack_history[user_id] = {(ip, port): 1}

    # Continue attack execution...
...
async def set_public_mode(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id

    if user_id != ADMIN_USER_ID:
        await context.bot.send_message(
            chat_id=update.effective_chat.id,
            text="*❌ You are not authorized to change public mode!*",
            parse_mode='Markdown'
        )
        return
    ...
    if len(context.args) == 0:
        await context.bot.send_message(
            chat_id=update.effective_chat.id,
            text="*⚠️ Usage: /set_public_mode <on/off> [time]*",
            parse_mode='Markdown'
        )
        return
    ...
    if mode not in ["on", "off"]:
        await context.bot.send_message(
            chat_id=update.effective_chat.id,
            text="*⚠️ Invalid mode! Use /set_public_mode on <time> or /set_public_mode off*",
            parse_mode='Markdown'
        )
        return
    ...
    if mode == "off":
        settings_collection.update_one(
            {"setting": "public_mode"},
            {"$set": {"value": False, "expiry": None}},
            upsert=True
        )
        await context.bot.send_message(
            chat_id=update.effective_chat.id,
            text="*✅ Public Mode is now OFF. Only authorized users can use the bot.*",
            parse_mode='Markdown'
        )
        return
    ...
    if len(context.args) > 1:
        ...
        try:
            ...
            if unit == "m":
                expiry_time = datetime.now(timezone.utc) + timedelta(minutes=value)
            elif unit == "h":
                expiry_time = datetime.now(timezone.utc) + timedelta(hours=value)
            elif unit == "d":
                expiry_time = datetime.now(timezone.utc) + timedelta(days=value)
            else:
                raise ValueError("Invalid time format. Use `m`, `h`, or `d`.")
        except Exception as e:
            await context.bot.send_message(
                chat_id=update.effective_chat.id,
                text="*⚠️ Invalid time format! Use `10m` (minutes), `2h` (hours), `1d` (days).*",
                parse_mode='Markdown'
            )
            return
    ...
    settings_collection.update_one(
        {"setting": "public_mode"},
        {"$set": {"value": True, "expiry": expiry_time}},
        upsert=True
    )

    expiry_text = f" until {expiry_time.strftime('%Y-%m-%d %H:%M:%S UTC')}" if expiry_time else " indefinitely"
    await context.bot.send_message(
        chat_id=update.effective_chat.id,
        text=f"*✅ Public Mode is now ON{expiry_text}. Anyone can use the bot.*",
        parse_mode='Markdown'
    )

    if expiry_time:
        asyncio.create_task(schedule_public_mode_off(expiry_time, context.bot))
