...
async def start(update: Update, context: CallbackContext):
    ...
    message = (
        "*🔥 Welcome to NeoModEngine DDOS Bot! 🔥*\n"
        ...
        "*💬 Owner: @NeoModEngine*\n"
        "*📢 Channel: [NeoModEngine Channel](https://t.me/+2Dq2ZEpde8QzY2U1)*"
    )
...
async def help_command(update: Update, context: CallbackContext):
    chat_id = update.effective_chat.id
    message = (
        "*❓ How to Use the NeoModEngine DDOS Bot?*\n\n"
        "*🔑 Commands:*\n\n"
        ...
        "*3. Contact the bot owner @NeoModEngine for assistance or clarifications.*\n\n"
        "*⚡ Owner: @NeoModEngine*\n"
        "*📢 Channel: [NeoModEngine Channel](https://t.me/+2Dq2ZEpde8QzY2U1)*"
    )
    await context.bot.send_message(chat_id=chat_id, text=message, parse_mode='Markdown')
...
async def approve(update: Update, context: CallbackContext):
    ...
    if chat_id != ADMIN_USER_ID:
        await context.bot.send_message(chat_id=chat_id, text="*⚠️ You need to get permission to use this command. Contact @NeoModEngine.*", parse_mode='Markdown')
        return
...
async def attack(update: Update, context: CallbackContext):
    ...
    if user_id not in users:
        await context.bot.send_message(chat_id=chat_id, text="*🤡 You need to get permission to use this bot. Contact owner @NeoModEngine.*", parse_mode='Markdown')
        return
